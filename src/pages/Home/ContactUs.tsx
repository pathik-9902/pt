      ...
      <div className="relative z-10 container mx-auto px-6">
            ...
            <div className="space-y-8">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    <input name="name" type="text" value={form.name} onChange={handleChange} placeholder="Enter name" required className="w-full rounded-full px-8 py-5 bg-white/5 border border-white/10 text-white placeholder-white/20 focus:border-[white] outline-none transition-all font-medium italic" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-[white] pl-6">Email Address</label>
                    <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="email@address.com" required className="w-full rounded-full px-8 py-5 bg-white/5 border border-white/10 text-white placeholder-white/20 focus:border-[white] outline-none transition-all font-medium italic" />
                  </div>
               </div>

               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-[white] pl-6">Phone</label>
                    <input name="phone" type="text" value={form.phone} onChange={handleChange} placeholder="+91" required className="w-full rounded-full px-8 py-5 bg-white/5 border border-white/10 text-white placeholder-white/20 focus:border-[white] outline-none transition-all font-medium italic" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-[white] pl-6">Location</label>
                    <input name="address" type="text" value={form.address} onChange={handleChange} placeholder="City, Country" required className="w-full rounded-full px-8 py-5 bg-white/5 border border-white/10 text-white placeholder-white/20 focus:border-[white] outline-none transition-all font-medium italic" />
                  </div>
               </div>
               ...
               <div className="pt-6">
                <motion.button
                  type="submit"
                  disabled={loading}
                  className="w-full rounded-full py-6 font-black uppercase tracking-[0.3em] text-white bg-[white] hover:bg-[white]/80 shadow-2xl transition-all disabled:opacity-50"
        ...
        >
          {[
            ...
            <div
              key={idx}
              className="glass-card rounded-[40px] p-10 group hover:border-[white]/30 transition-all duration-500 shadow-xl"
            >
              <div className="w-14 h-14 rounded-2xl bg-[white]/10 flex items-center justify-center mb-6 group-hover:bg-[white]/20 transition-all">
                <item.icon className="w-6 h-6 text-[white]" />
              </div>
              <div className="text-[10px] font-black text-[white] uppercase tracking-widest mb-2 opacity-60 leading-none">{item.label}</div>
