...
const reasons = [
  { id: 1, icon: <Wrench className="w-6 h-6 text-[white]" />, text: "State-of-the-art in-house manufacturing" },
  { id: 2, icon: <Award className="w-6 h-6 text-[white]" />, text: "ISO 9001:2015 certified quality systems" },
  { id: 3, icon: <ShieldCheck className="w-6 h-6 text-[white]" />, text: "Metallurgically & dimensionally precise castings" },
  { id: 4, icon: <Users className="w-6 h-6 text-[white]" />, text: "Experienced engineers and skilled workforce" },
];
  ...
  return (
    <section className="relative py-32 overflow-hidden">
      <div className="relative z-10 container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        ...
        >
          <div className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-6 py-2 text-xs font-black tracking-[0.2em] text-[white] uppercase backdrop-blur-xl mb-10">
          ...
          <p className="text-xl text-gray-300 font-medium italic mb-12 opacity-90 leading-relaxed border-l-4 border-[white] pl-8">
          ...
          <ul className="grid sm:grid-cols-2 gap-6 mb-12">
            {reasons.map((reason, index) => (
              <motion.li
                ...
                className="glass-card flex items-center gap-5 p-6 rounded-[32px] hover:border-[white]/30 transition-all group shadow-xl"
              >
                <div className="w-12 h-12 rounded-2xl bg-[white]/10 flex items-center justify-center group-hover:bg-[white]/20 transition-colors shadow-inner">
                  {reason.icon}
                </div>
        ...
        <div className="grid grid-cols-2 gap-6 relative">
          <div className="absolute -inset-10 bg-[white]/5 blur-[100px] rounded-full pointer-events-none" />
          <motion.div
