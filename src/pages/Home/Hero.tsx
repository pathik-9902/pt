...
export default function Hero(): JSX.Element {
    ...
    customPaging: (i: number) => (
      <div
        className={`h-1.5 transition-all duration-500 rounded-full ${
          i === active ? "w-8 bg-[#7FA1C3] shadow-[0_0_15px_rgba(255,255,255,0.6)]" : "w-1.5 bg-white/20"
        }`}
      />
    ),
  };
  ...
  return (
    <section className="relative w-full h-[85vh] md:h-screen overflow-hidden">
      ...
      {/* Ambient Glows */}
      <div className="absolute -top-1/4 -left-1/4 w-1/2 h-1/2 bg-[#7FA1C3]/10  rounded-full animate-pulse" />
      <div className="absolute -bottom-1/4 -right-1/4 w-1/2 h-1/2 bg-[#7FA1C3]/10  rounded-full animate-pulse" />
                  ...
                  >
                    <h1 className="text-4xl sm:text-6xl md:text-8xl font-black text-white tracking-tighter leading-[0.9] mb-8 uppercase italic">
                      {slide.heading.split(' ').map((word, i) => (
                        <span key={i} className={i % 2 !== 0 ? "text-[#7FA1C3] block sm:inline" : "block sm:inline"}>
                          {word}{' '}
                        </span>
                      ))}
                    </h1>
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
                      <Link
                        to="/contact"
                        className="liquid-button bg-white text-black px-10 py-5 text-lg font-black uppercase tracking-widest hover:bg-[#7FA1C3] hover:text-white transition-all shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center group"
                      >
                        Start Project
